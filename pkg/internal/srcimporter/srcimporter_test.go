...
var importedObjectTests = []struct {
	...
	// NB(charlie): we can't use "golang.org/x/text/unicode" since we don't import it
	// so instead we use "golang.org/x/tools" which we do import.
	// {"golang.org/x/text/unicode/norm.MaxSegmentSize", "const MaxSegmentSize untyped int"},
	{"golang.org/x/tools/go/gcexportdata.Find", "func Find(importPath string, srcDir string) (filename string, path string)"},
}
